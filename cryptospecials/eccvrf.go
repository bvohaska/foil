...
*  Beta = H_2(lambda^f)
 */
func (rep ECCVRF) Generate(h hash.Hash, ec elliptic.Curve, alpha []byte, verbose bool) (eccPub ECCPoint, eccProof Proof, beta []byte, err error) {
	...
	*  validated it will be optimized (removal of unneeded big.Int, new(), etc)
	...
	// Randomly select a secret key: x (mod q)
	x, err = rand.Int(rand.Reader, ec.Params().N)
	if err != nil {
		return ECCPoint{}, Proof{}, nil, err
	}
	x.Mod(x, ec.Params().N)

	// Generate public key: g^x => x * (xGx, yGx)
	...
	*				= H_2(xh1, xh2)
	...
	return eccPub, eccProof, beta, nil
}
 ...
 */
func (rep ECCVRF) Verify(h hash.Hash, pubK *ECCPoint, ec elliptic.Curve, alpha []byte, beta []byte, eccProof *Proof, verbose bool) (valid bool, err error) {
	...
	*			= k*G = G^k
	...
	swap.x, swap.y = ec.ScalarMult(ec.Params().Gx, ec.Params().Gy, eccProof.s.Bytes())
